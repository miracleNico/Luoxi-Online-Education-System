...
import com.luoxi.constant.ConstRoleType;
import com.luoxi.exception.LxException;
import com.luoxi.model.Role;
...
public class RoleService implements IRoleService{
	...
	public void saveRole(ReqSaveRole req) throws Exception {
		...
		if(StringUtils.isNotBlank(req.getRoleId())) {
			if(!BeanUtil.isEmpty(dbRole) && !dbRole.getRoleId().equals(req.getRoleId())) throw LxException.of().setMsg("角色已存在");
			...
			if(!BeanUtil.isEmpty(dbRole)) throw LxException.of().setMsg("角色已存在");
